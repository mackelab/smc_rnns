...
def predict(vae,u=None,x=None,dur=None,initial_state="prior_sample", observation_model="Gauss",optimal_proposal=False,
            verbose=False, sim_v=True,cut_off=0):
    with torch.no_grad():
        if u is None:
        ...
        if isinstance(initial_state, str):
            ...
            z0 = initial_state
        if sim_v:
            Z, v = vae.rnn.get_latent_time_series(time_steps=dur, z0=z0, u=u, noise_scale=1,sim_v=sim_v, cut_off=cut_off)
        else:
            Z = vae.rnn.get_latent_time_series(time_steps=dur, z0=z0, u=u, noise_scale=1,sim_v=sim_v, cut_off=cut_off)
            v = u.unsqueeze(-1)
        rates = (
